...
In computer vision, different tasks exists as introduced in our previous blogpost [Segmentation vs Detection vs Classification in Computer Vision: A Comparative Analysis](/post/segmentation-vs-detection-vs-classification-in-computer-vision-a-comparative-analysis). As explained in the article, there are two types of segmentation, semantic segmentation and instance segmentation. In this article we will discover one of the most powerful archtecture for instance segmentation, which is Mask R-CNN.
...
During inference, Mask R-CNN applies the [trained model ](/experiment-tracking)to unseen images, performing region proposals, classification, bounding box regression, and mask prediction in a unified manner. The result is accurate instance segmentation, with objects distinctly outlined and labeled within the image.
