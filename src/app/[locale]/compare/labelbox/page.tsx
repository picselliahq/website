...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'compare.labelbox.metadata' });
  const canonical = localizedUrl("/compare/labelbox", locale);
  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical,
      languages: localizedAlternates("/compare/labelbox"),
    },
    openGraph: {
      title: t('ogTitle'),
...
const featureCompetitorStatuses: Record<string, string[]> = {
  dataManagement: ["partial","full","partial","full","full"],
  annotationLabeling: ["full","full","full","full","full"],
  trainingExperimentation: ["none","none","none","none","none"],
...
export default async function CompareLabelboxPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'compare.labelbox' });
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-6 md:gap-10 mb-10">
              ...
              <div className="flex flex-col items-center">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl overflow-hidden mb-3 shadow-lg shadow-[var(--system-purple)]/10">
                  <Image src="/images/compare/labelbox.svg" alt="Labelbox" width={80} height={80} className="w-full h-full" />
                </div>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold mb-6 tracking-tight">
              {t('heroTitle')}{" "}
              <span className="text-[var(--system-purple)]">{t('heroHighlight')}</span>
            </h1>
          ...
          <div className="mt-12 max-w-2xl mx-auto space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 rounded overflow-hidden">
                    <Image src="/images/compare/labelbox.svg" alt="Labelbox" width={20} height={20} className="w-full h-full" />
                  </div>
                  <span className="text-sm font-medium text-[var(--label)]">Labelbox</span>
                </div>
                <span className="text-sm font-semibold text-[var(--system-purple)]">{t('competitorStages')}</span>
              </div>
              <div className="h-3 rounded-full bg-[var(--tertiary-system-background)] overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-[var(--system-purple)] to-[var(--system-purple)]/70" style={{ width: "33%" }} />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="card p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--system-purple)]">{t('considerCompetitorSub')}</p>
                </div>
              </div>
              <div className="space-y-3">
