...
const roboflowStatuses = ["full", "partial", "partial", "partial", "none", "partial"] as const;
...
const featureCompetitorStatuses: Record<string, string[]> = {
  dataManagement: ["partial","none","none","partial","none"],
  annotationLabeling: ["partial","partial","none","full"],
  trainingExperimentation: ["partial","none","partial","partial"],
  deploymentMonitoring: ["partial","none","none","none"],
  enterpriseCompliance: ["partial","none","partial","partial","none"],
};
...
export default async function CompareRoboflowPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations({ locale, namespace: 'compare.roboflow' });
  ...
  const comparisonCategories = categoryKeys.map((key, i) => ({
    ...
    features: (t.raw(`comparison.${key}.features`) as Array<{feature: string; picselliaNote: string; competitorNote: string}>).map((f, fi) => ({
      ...
      roboflow: featureCompetitorStatuses[key][fi],
      roboflowNote: f.competitorNote,
    })),
  }));
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-6 md:gap-10 mb-10">
              ...
              <div className="flex flex-col items-center">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl overflow-hidden mb-3 shadow-lg shadow-[var(--system-blue)]/10">
                  <Image src="/images/compare/roboflow.svg" alt="Roboflow" width={80} height={80} className="w-full h-full" />
                </div>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold mb-6 tracking-tight">
              {t('heroTitle')}{" "}
              <span className="text-[var(--system-blue)]">{t('heroHighlight')}</span>
            </h1>
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
            {pipelineStages.map((stage) => {
              ...
              const rColor = stage.roboflow === "full" ? "var(--picsellia-green)" : stage.roboflow === "partial" ? "var(--system-orange)" : "var(--system-red)";
              return (
          ...
          <div className="mt-12 max-w-2xl mx-auto space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-semibold text-[var(--system-blue)]">{t('competitorStages')}</span>
              </div>
              <div className="h-3 rounded-full bg-[var(--tertiary-system-background)] overflow-hidden">
            ...
            <div className="card p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--system-blue)]">{t('considerCompetitorSub')}</p>
                </div>
              </div>
              <div className="space-y-3">
