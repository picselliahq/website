...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: localizedAlternates("/enterprise"),
    },
    openGraph: {
      title: t('ogTitle'),
...
const certifications = [
  { name: 'ISO 27001:2022', description: 'International standard for information security management', iconPath: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  { name: 'GDPR Compliant', description: 'EU data protection & privacy standards', iconPath: 'M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3' },
  { name: 'Data Encryption', description: 'AES-256 at rest, TLS 1.3 in transit', iconPath: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z' },
  { name: 'SSO / SAML', description: 'Enterprise identity provider integration', iconPath: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
];

// Security features
const securityFeatures = [
  { title: 'Audit Logs', description: 'Complete activity tracking for compliance and forensics', iconPath: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  { title: 'API Key Management', description: 'Scoped tokens with expiration and revocation', iconPath: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
  { title: 'IP Allowlisting', description: 'Restrict access to trusted networks', iconPath: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9' },
  { title: '2FA / MFA', description: 'Multi-factor authentication for all users', iconPath: 'M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z' },
  { title: 'Data Isolation', description: 'Tenant separation at storage and compute level', iconPath: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
  { title: 'Backup & Recovery', description: 'Automated backups with point-in-time recovery', iconPath: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
];

// Enterprise value propositions
const enterpriseValues = [
  { title: 'Dedicated Support', description: 'Named customer success manager, priority SLAs, and direct engineering access', iconPath: 'M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z' },
  { title: 'Custom Integrations', description: 'Connect with your existing CI/CD, identity providers, and monitoring stack', iconPath: 'M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z' },
  { title: 'Training & Onboarding', description: 'White-glove onboarding, team training sessions, and ongoing enablement', iconPath: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
];
...
export default async function EnterprisePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tDeploymentOptions = t.raw('deploymentOptions') as Array<{name: string; description: string; tagline: string; features: string[]; bestFor: string}>;
  const deployColors = ['var(--picsellia-green)', 'var(--system-blue)', 'var(--system-indigo)'];
  const deployIcons = [
  ...
  const tCertifications = t.raw('certifications') as Array<{name: string; description: string}>;
  const certIcons = [
  ...
  const tSecurityFeatures = t.raw('securityFeatures') as Array<{title: string; description: string}>;
  const secFeatIcons = [
  ...
  const tRbacRoles = t.raw('rbacRoles') as Array<{level: string; roles: Array<{name: string; description: string}>}>;
  const rbacColors = ['var(--system-indigo)', 'var(--system-blue)', 'var(--picsellia-green)'];

  const tEnterpriseValues = t.raw('enterpriseValues') as Array<{title: string; description: string}>;
  const evIcons = [
  ...
  const tHybridItems = t.raw('hybridItems') as Array<{label: string; desc: string}>;
  const hybridIcons = [
  ...
  const tInfraItems = t.raw('infraItems') as string[];
  const tControlPlaneItems = t.raw('controlPlaneItems') as string[];
  const tOnPremFeatures = t.raw('onPremFeaturesList') as string[];
  const tOnPremTags = t.raw('onPremTags') as string[];

  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: 'Enterprise', url: '/enterprise' }], locale)} />
      {/* Hero Section */}
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--system-indigo)]/10 border border-[var(--system-indigo)]/20 mb-8">
              ...
              <span className="text-sm font-medium text-[var(--system-indigo)]">{t('badge')}</span>
            </div>

            <h1 className="text-4xl sm:text-5xl md:text-6xl font-semibold mb-6 tracking-tight max-w-4xl mx-auto">
              {t('heroTitle')}<span className="text-[var(--system-indigo)]">{t('heroHighlight')}</span>
            </h1>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-[var(--system-blue)] text-sm font-medium uppercase tracking-wider mb-3 block">
              {t('deploymentOptionsLabel')}
            </span>
            <h2 className="text-3xl md:text-4xl font-semibold mb-4">
              {t('deploymentOptionsTitle')}
            </h2>
            <p className="text-[var(--secondary-label)] max-w-2xl mx-auto">
              {t('deploymentOptionsDescription')}
            </p>
          </div>

          <div className="grid lg:grid-cols-3 gap-6">
            {tDeploymentOptions.map((option, oi) => (
              <div key={option.name} className="card p-0 overflow-hidden flex flex-col">
                <div className="p-6 border-b border-[var(--border)]">
                  <div
                    className="w-14 h-14 rounded-xl flex items-center justify-center mb-4"
                    style={{ backgroundColor: `color-mix(in srgb, ${deployColors[oi]} 15%, transparent)`, color: deployColors[oi] }}
                  >
                    <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={deployIcons[oi]} />
                    </svg>
                  </div>
                  ...
                  <div className="space-y-3">
                    {option.features.map((feature) => (
                      <div key={feature} className="flex items-start gap-2">
                        <svg className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: deployColors[oi] }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="p-4 border-t border-[var(--border)] bg-[var(--tertiary-system-background)]">
                  <p className="text-xs text-[var(--tertiary-label)]">
                    <span className="font-medium text-[var(--secondary-label)]">{t('bestFor')}</span> {option.bestFor}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
                  ...
                  <div key={item.label} className="flex items-start gap-4 p-4 rounded-lg bg-[var(--tertiary-system-background)] border border-[var(--border)]">
                    <div className="w-10 h-10 rounded-lg bg-[var(--system-blue)]/10 flex items-center justify-center text-[var(--system-blue)] flex-shrink-0">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={hybridIcons[hi]} />
                      </svg>
                    </div>
                    <div>
            ...
            <div className="card p-6">
              <div className="text-xs font-medium text-[var(--tertiary-label)] uppercase tracking-wider mb-6">{t('archOverview')}</div>

              <div className="space-y-4">
                <div className="p-4 rounded-lg border-2 border-dashed border-[var(--system-blue)]/30 bg-[var(--system-blue)]/5">
                  <div className="text-xs font-semibold text-[var(--system-blue)] mb-3">{t('yourInfra')}</div>
                  <div className="grid grid-cols-3 gap-2">
                    {tInfraItems.map((item) => (
                ...
                <div className="p-4 rounded-lg border border-[var(--picsellia-green)]/30 bg-[var(--picsellia-green)]/5">
                  <div className="text-xs font-semibold text-[var(--picsellia-green)] mb-3">{t('controlPlaneName')}</div>
                  <div className="grid grid-cols-2 gap-2">
                    {tControlPlaneItems.map((item) => (
              ...
              <div className="mt-6 p-3 rounded-lg bg-[var(--tertiary-system-background)]">
                <div className="flex items-center gap-2 text-xs text-[var(--secondary-label)]">
                  ...
                  <span>{t('dataNeverLeavesMsg')}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* On-Premise Enterprise Section */}
      <section className="py-24 border-b border-[var(--border)]">
        <div className="max-w-6xl mx-auto px-6">
          {/* Enterprise On-Prem Banner */}
          <div className="card p-8 md:p-12 mb-16 relative overflow-hidden border-[var(--system-indigo)]/20">
            ...
            <div className="relative text-center">
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-8">
                {tOnPremTags.map(tag => (
                  <span key={tag} className="px-4 py-2 rounded-full text-xs font-medium bg-[var(--system-indigo)]/10 text-[var(--system-indigo)] border border-[var(--system-indigo)]/20">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="order-2 lg:order-1">
              <div className="grid grid-cols-2 gap-4">
                <div className="card p-6">
                  ...
                  <h3 className="text-lg font-semibold text-[var(--label)] mb-2">{t('k8sTitle')}</h3>
                ...
                <div className="card p-6">
                  ...
                  <h3 className="text-lg font-semibold text-[var(--label)] mb-2">{t('dockerTitle')}</h3>
                  <p className="text-xs text-[var(--tertiary-label)] mb-4">
                    {t('dockerDesc')}
                  </p>
                  <div className="font-mono text-[10px] bg-[var(--black)] rounded p-2 text-[var(--secondary-label)]">
              ...
              <div className="mt-6 p-4 rounded-lg border border-[var(--border)] bg-[var(--tertiary-system-background)]">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-[var(--label)] mb-1">{t('airGapTitle')}</div>
                    <p className="text-xs text-[var(--tertiary-label)]">
                      {t('airGapDesc')}
                    </p>
                  </div>
            ...
            <div className="order-1 lg:order-2">
              ...
              <h2 className="text-3xl md:text-4xl font-semibold mb-6">
                {t('onPremControlLabel')}
              </h2>
              <p className="text-[var(--secondary-label)] mb-8">
                {t('onPremControlDesc2')}
              </p>

              <div className="space-y-3">
                {tOnPremFeatures.map((feature) => (
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-16">
            {tEnterpriseValues.map((item, evi) => (
              <div key={item.title} className="card p-6">
                <div className="w-10 h-10 rounded-lg bg-[var(--system-indigo)]/10 flex items-center justify-center mb-4 text-[var(--system-indigo)]">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={evIcons[evi]} />
                  </svg>
                </div>
                <h4 className="text-sm font-semibold text-[var(--label)] mb-1">{item.title}</h4>
                <p className="text-xs text-[var(--tertiary-label)]">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-[var(--picsellia-green)] text-sm font-medium uppercase tracking-wider mb-3 block">
              {t('complianceSectionLabel')}
            </span>
            <h2 className="text-3xl md:text-4xl font-semibold mb-4">
              {t('complianceSectionTitle')}
            </h2>
            <p className="text-[var(--secondary-label)] max-w-2xl mx-auto">
              {t('complianceSectionDesc')}
            </p>
          </div>

          <div className="grid md:grid-cols-4 gap-6 mb-16">
            {tCertifications.map((cert, ci) => (
              <div key={cert.name} className="card p-6 text-center">
                <div className="w-14 h-14 rounded-xl bg-[var(--picsellia-green)]/10 flex items-center justify-center mx-auto mb-4 text-[var(--picsellia-green)]">
                  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={certIcons[ci]} />
                  </svg>
                </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {tSecurityFeatures.map((feature, sfi) => (
              <div key={feature.title} className="flex items-start gap-3 p-4 rounded-lg border border-[var(--border)]">
                <div className="w-9 h-9 rounded-lg bg-[var(--tertiary-system-background)] flex items-center justify-center flex-shrink-0 text-[var(--secondary-label)]">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={secFeatIcons[sfi]} />
                  </svg>
                </div>
                <div>
      ...
      <section className="py-24 border-b border-[var(--border)] relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          <div className="text-center mb-16">
            <span className="text-[var(--system-indigo)] text-sm font-medium uppercase tracking-wider mb-3 block">
              {t('accessControlSectionLabel')}
            </span>
            <h2 className="text-3xl md:text-4xl font-semibold mb-4">
              {t('accessControlSectionTitle')}
            </h2>
            <p className="text-[var(--secondary-label)] max-w-2xl mx-auto">
              {t('accessControlSectionDesc')}
            </p>
          </div>

          <div className="grid lg:grid-cols-3 gap-6">
            {tRbacRoles.map((level, li) => (
              <div key={level.level} className="card p-0 overflow-hidden">
                <div
                  className="px-6 py-4 border-b border-[var(--border)]"
                  style={{ backgroundColor: `color-mix(in srgb, ${rbacColors[li]} 8%, transparent)` }}
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: rbacColors[li] }} />
                    <h3 className="text-lg font-semibold text-[var(--label)]">{level.level} {t('levelSuffix')}</h3>
                  </div>
                </div>
          ...
          <div className="mt-12 card p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-[var(--label)] mb-1">{t('customRolesLabel')}</h3>
