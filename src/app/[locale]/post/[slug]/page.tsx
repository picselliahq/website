...
export default async function BlogPostPage({ params }: Props) {
  ...
  const tCta = await getTranslations({ locale, namespace: 'blogCta' });
  const post = getPostBySlug(slug, locale);
  if (!post) notFound();
  ...
  const conversionCopy = getConversionCopy(post.frontmatter.category, tCta);
  ...
  const related = allPosts
    .filter(
      (p) =>
        p.slug !== slug && p.frontmatter.category === post.frontmatter.category,
    )
    .slice(0, 3);

  return (
    <article className="py-24 px-6">
      <JsonLd data={articleJsonLd(post, slug, locale)} />
      <JsonLd
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <nav className="mb-8 text-sm text-secondary">
          <Link href="/blog" className="hover:text-label transition-colors">
            {t('breadcrumbBlog')}
          </Link>
        ...
        <header className="max-w-3xl mb-12">
          ...
          <h1
            className="text-3xl md:text-4xl lg:text-5xl font-semibold text-label mb-6"
        ...
        <div className="flex gap-12">
          ...
          <div className="flex-1 min-w-0 max-w-3xl">
            {/* Conversion CTA */}
            <BlogConversionCTA copy={conversionCopy} />
        ...
        {related.length > 0 && (
          <div className="mt-20">
            <h2 className="text-2xl font-semibold text-label mb-8">
              {t('relatedArticles')}
            </h2>
