...
const modules = [
  {
    ...
    nameKey: "dataEngineName" as const,
    descKey: "dataEngineDesc" as const,
    monthlyPrice: 1150,
    annualPrice: 12000,
  ...
  },
  {
    ...
    nameKey: "visionaiFactoryName" as const,
    descKey: "visionaiFactoryDesc" as const,
    monthlyPrice: 288,
    annualPrice: 3000,
    extraUserMonthly: 86,
    extraUserAnnual: 900,
  ...
  },
  {
    ...
    nameKey: "reliabilityEngineName" as const,
    descKey: "reliabilityEngineDesc" as const,
    monthlyPrice: 575,
    annualPrice: 6000,
...
const muTiersNumeric = [
  ...
  { max: 100000, price: 0.4 },
];

function getTierPrice(
  volume: number,
  tiers: { max: number; price: number }[],
): number {
...
export default function PricingCalculator({
  ...
  function toggleModule(id: ModuleId) {
    setSelectedModules((prev) => {
      ...
      if (id === "data-engine") {
        ...
        if (
          prev["data-engine"] &&
          (prev["visionai-factory"] || prev["reliability-engine"])
        )
          return prev;
  ...
  const estimate = useMemo(() => {
    ...
    for (const mod of modules) {
      ...
      const base =
        billingCycle === "monthly" ? mod.monthlyPrice : mod.annualPrice / 12;
      items.push({ label: t(mod.nameKey), amount: base, color: mod.color });

      const users = extraUsers[mod.id];
      if (users > 0) {
        const userCost =
          billingCycle === "monthly"
            ? mod.extraUserMonthly * users
            : (mod.extraUserAnnual / 12) * users;
        items.push({
          label: t('extraUsersLabel', { name: t(mod.nameKey), count: users }),
          amount: userCost,
          color: mod.color,
        });
      }
    ...
    if (labelerSeats > 0) {
      const labelerCost =
        billingCycle === "monthly" ? labelerSeats * 10 : labelerSeats * 10;
      items.push({
        label: t('labelerSeatsLabel', { count: labelerSeats }),
        amount: labelerCost,
    ...
    if (dpuVolume > 0 && selectedModules["data-engine"]) {
      ...
      items.push({
        label: t('dpuUsageLabel', { count: formatNumber(dpuVolume) }),
        amount: dpuVolume * unitPrice,
        color: "var(--system-blue)",
      });
    }
    ...
    if (muVolume > 0 && selectedModules["reliability-engine"]) {
      ...
      items.push({
        label: t('muUsageLabel', { count: formatNumber(muVolume) }),
        amount: muVolume * unitPrice,
        color: "var(--system-orange)",
      });
    }
    ...
    if (gpuHours > 0) {
      items.push({
        label: t('gpuComputeLabel', { hours: gpuHours }),
        amount: gpuHours * 3.5,
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="card p-6">
              <h3 className="text-sm font-semibold text-[var(--label)] mb-4 uppercase tracking-wider">
                {t('selectModules')}
              </h3>
                ...
                {modules.map((mod) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderLeftColor: selected ? mod.color : "var(--border)",
                      }}
                    >
            ...
            <div className="card p-6">
              ...
              <div className="space-y-4">
                {modules.map((mod) => {
                  ...
                  const userCost =
                    billingCycle === "monthly"
                      ? mod.extraUserMonthly
                      : Math.round(mod.extraUserAnnual / 12);
                  return (
                    <div
                      key={mod.id}
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[10px] text-[var(--tertiary-label)]">
                          {formatCurrency(userCost)}{t('perUserMonth')}
                        </div>
                      </div>
                      <Stepper
                        value={extraUsers[mod.id]}
                        onChange={(v) =>
                          setExtraUsers((prev) => ({ ...prev, [mod.id]: v }))
                        }
                        max={50}
                      />
                    </div>
                ...
                <div className="pt-4 border-t border-[var(--border)] flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[10px] text-[var(--tertiary-label)]">
                      {formatCurrency(10)}{t('perSeatMonth')}
                    </div>
                  </div>
                  <Stepper
                    value={labelerSeats}
                    onChange={setLabelerSeats}
                    max={100}
                  />
                </div>
              </div>
              ...
              {selectedModules["data-engine"] && (
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-sm text-[var(--label)]">
                      {t('dpuLabel')}
                    </label>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      value={dpuVolume}
                      onChange={(e) =>
                        setDpuVolume(Math.max(0, Number(e.target.value)))
                      }
                    ...
                    {dpuVolume > 0 && (
                      <span className="ml-auto text-xs font-mono text-[var(--system-blue)]">
                        @ \u20ac
                        {getTierPrice(dpuVolume * 12, dpuTiersNumeric).toFixed(
                          3,
                        )}
                        {t('perDpu')}
                      </span>
                  ...
                  {dpuVolume > 0 && (
                    <p className="text-xs text-[var(--tertiary-label)] mt-2">
                      {t('dpuCorresponds', { count: formatNumber(dpuVolume * 60) })}
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      value={muVolume}
                      onChange={(e) =>
                        setMuVolume(Math.max(0, Number(e.target.value)))
                      }
                  ...
                  {muVolume > 0 && (
                    <p className="text-xs text-[var(--tertiary-label)] mt-2">
                      {t('muCorresponds', { count: formatNumber(muVolume * 1000) })}
                    </p>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    value={gpuHours}
                    onChange={(e) =>
                      setGpuHours(Math.max(0, Number(e.target.value)))
                    }
          ...
          <div className="lg:col-span-2">
            <div className="card p-6 sticky top-28">
              <h3 className="text-sm font-semibold text-[var(--label)] mb-6 uppercase tracking-wider">
                {t('monthlyEstimate')}
              </h3>

              {estimate.items.length === 0 ? (
                <p className="text-sm text-[var(--tertiary-label)] py-8 text-center">
                  {t('emptyState')}
                </p>
              ) : (
                <>
                  ...
                  <div className="border-t border-[var(--border)] pt-4 mb-6">
                    ...
                    <div className="text-xs text-[var(--tertiary-label)] text-right mt-1">
                      {t('perMonthSuffix')}
                    </div>
                    {billingCycle === "annual" && (
                      <div className="text-xs text-[var(--picsellia-green)] text-right mt-1">
                        {t('annualBilled', { amount: formatCurrency(estimate.total * 12) })}
                      </div>
                    )}
                  </div>
                </>
...
function Stepper({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-sm w-8 text-center text-[var(--label)]">
        {value}
      </span>
      <button
