...
export default async function AbelioCaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const td = await getTranslations({ locale, namespace: 'useCases.abelio.data' });

  const localizedData: CaseStudyData = {
    ...abelioData,
    company: { ...abelioData.company, industry: td('industry'), description: td('description') },
    headline: td('headline'),
    subheadline: td('subheadline'),
    metrics: (td.raw('metrics') as Array<{value: string; label: string; description: string}>),
    transformation: {
      before: { title: td('beforeItems.0', { defaultValue: '' }), items: td.raw('beforeItems') as string[] },
      after: { title: td('afterItems.0', { defaultValue: '' }), items: td.raw('afterItems') as string[] },
    },
    challengeIntro: td('challengeIntro'),
    challenges: td.raw('challenges') as Array<{title: string; description: string}>,
    workflow: (td.raw('workflow') as Array<{title: string; description: string; feature: string}>).map((w, i) => ({
      ...abelioData.workflow[i],
      title: w.title,
      description: w.description,
      feature: w.feature,
    })),
    solutionIntro: td('solutionIntro'),
    solutions: (td.raw('solutions') as Array<{title: string; description: string; feature: string; metricsValue?: string; metricsLabel?: string}>).map((s, i) => ({
      ...abelioData.solutions[i],
      title: s.title,
      description: s.description,
      feature: s.feature,
      metrics: s.metricsValue ? { value: s.metricsValue, label: s.metricsLabel || '' } : undefined,
    })),
    quote: { ...abelioData.quote, text: td('quote'), author: td('quoteAuthor'), role: td('quoteRole') },
    resultsIntro: td('resultsIntro'),
    results: td.raw('results') as Array<{title: string; description: string}>,
    featuresUsed: (td.raw('featuresUsed') as Array<{name: string; description: string}>).map((f, i) => ({
      ...abelioData.featuresUsed[i],
      name: f.name,
      description: f.description,
    })),
    relatedCaseStudies: (td.raw('relatedStudies') as Array<{title: string; company: string; industry: string}>).map((r, i) => ({
      ...abelioData.relatedCaseStudies![i],
      title: r.title,
      company: r.company,
      industry: r.industry,
    })),
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          { name: "Customer Stories", url: "/use-cases" },
          { name: "Abelio", url: "/use-cases/abelio" },
        ], locale)}
      />
      <CaseStudyTemplate data={localizedData} />
    </>
  );
}
