...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'useCases.altaroad.metadata' });
  const canonical = localizedUrl("/use-cases/altaroad", locale);
  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical,
      languages: localizedAlternates("/use-cases/altaroad"),
    },
    openGraph: {
      title: t('ogTitle'),
...
export default async function AltaroadCaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: 'Customer Stories', url: '/use-cases' }, { name: 'Altaroad', url: '/use-cases/altaroad' }], locale)} />
