...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'useCases.ficha.metadata' });
  const canonical = localizedUrl("/use-cases/ficha", locale);
  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical,
      languages: localizedAlternates("/use-cases/ficha"),
    },
    openGraph: {
      title: t('ogTitle'),
...
export default async function FichaCaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: 'Customer Stories', url: '/use-cases' }, { name: 'Ficha', url: '/use-cases/ficha' }], locale)} />
