...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'useCases.pellencst.metadata' });
  const canonical = localizedUrl("/use-cases/pellencst", locale);
  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical,
      languages: localizedAlternates("/use-cases/pellencst"),
    },
    openGraph: {
      title: t('ogTitle'),
...
export default async function PellencSTCaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: 'Customer Stories', url: '/use-cases' }, { name: 'PellencST', url: '/use-cases/pellencst' }], locale)} />
