...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'useCases.sgs.metadata' });
  const canonical = localizedUrl("/use-cases/sgs", locale);
  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical,
      languages: localizedAlternates("/use-cases/sgs"),
    },
    openGraph: {
      title: t('ogTitle'),
...
export default async function SGSCaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  return (
    <>
      <JsonLd data={breadcrumbJsonLd([{ name: 'Customer Stories', url: '/use-cases' }, { name: 'SGS', url: '/use-cases/sgs' }], locale)} />
