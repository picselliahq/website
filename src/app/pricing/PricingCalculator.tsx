  ...
  const estimate = useMemo(() => {
    ...
    for (const mod of modules) {
      ...
      if (users > 0) {
        ...
        items.push({
          label: `${mod.name} extra users (${users})`,
          amount: userCost,
          color: mod.color,
        });
      }
    ...
    if (labelerSeats > 0) {
      ...
      items.push({
        label: `Labeler seats (${labelerSeats})`,
        amount: labelerCost,
    ...
    if (dpuVolume > 0 && selectedModules["data-engine"]) {
      ...
      items.push({
        label: `DPU usage (${formatNumber(dpuVolume)}/mo)`,
    ...
    if (muVolume > 0 && selectedModules["reliability-engine"]) {
      ...
      items.push({
        label: `MU usage (${formatNumber(muVolume)}/mo)`,
      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        <div className="text-center mb-12">
          ...
          <p className="text-[var(--secondary-label)] max-w-2xl mx-auto">
            Configure your platform setup and usage to get an instant cost
            estimate.
          </p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="card p-6">
              ...
              <p className="text-xs text-[var(--tertiary-label)] mb-4">
                Each module includes 3 seats. Add more users below.
              </p>
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-[10px] text-[var(--tertiary-label)]">
                          {formatCurrency(userCost)}/user/mo
                        </div>
                      </div>
                      <Stepper
                ...
                <div className="mb-6">
                  ...
                  {dpuVolume > 0 && (
                    <p className="text-xs text-[var(--tertiary-label)] mt-2">
                      Corresponds to{" "}
                      <span className="font-mono text-[var(--secondary-label)]">
                        {formatNumber(dpuVolume * 60)}
                      </span>{" "}
                      new images every month
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="mb-6">
                  ...
                  {muVolume > 0 && (
                    <p className="text-xs text-[var(--tertiary-label)] mt-2">
                      Corresponds to{" "}
                      <span className="font-mono text-[var(--secondary-label)]">
                        {formatNumber(muVolume * 1000)}
                      </span>{" "}
                      monitored predictions every month
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="card p-6 sticky top-28">
              ...
              {estimate.items.length === 0 ? (
                <p className="text-sm text-[var(--tertiary-label)] py-8 text-center">
                  Select at least one module to see your estimate.
                </p>
              ) : (
                <>
                  ...
                  <div className="border-t border-[var(--border)] pt-4 mb-6">
                    ...
                    {billingCycle === "annual" && (
                      <div className="text-xs text-[var(--picsellia-green)] text-right mt-1">
                        {formatCurrency(estimate.total * 12)}/year billed
                        annually
                      </div>
                    )}
                  </div>
              ...
              <p className="text-[10px] text-[var(--tertiary-label)] mt-4 text-center">
                Estimates are approximate. Final pricing may vary based on
