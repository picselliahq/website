        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-6">
              <span className="text-xs font-medium text-[var(--system-indigo)]">DEPLOYMENT ARCHITECTURE</span>
              <span className="text-xs text-[var(--tertiary-label)]">Managed infrastructure</span>
            </div>

            <div className="space-y-4">
              {/* Ingress layer */}
              <div className="p-4 rounded-lg border border-[var(--system-blue)]/30 bg-[var(--system-blue)]/5">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 rounded-lg bg-[var(--system-blue)]/10 flex items-center justify-center">
                ...
                <div className="flex items-center gap-2 text-[10px] text-[var(--tertiary-label)]">
                  <span className="px-1.5 py-0.5 rounded bg-[var(--system-blue)]/10">HTTPS</span>
                  <span className="px-1.5 py-0.5 rounded bg-[var(--system-blue)]/10">API Keys</span>
                  <span className="px-1.5 py-0.5 rounded bg-[var(--system-blue)]/10">Rate Limiting</span>
                </div>
              </div>
              ...
              {/* Serving layer */}
              <div className="p-4 rounded-lg border border-[var(--system-indigo)]/30 bg-[var(--system-indigo)]/5">
                <div className="flex items-center gap-3 mb-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="p-2 rounded bg-[var(--system-indigo)]/10 text-center">
                      <div className="text-[10px] font-mono text-[var(--system-indigo)]">replica-{i}</div>
                      <div className="w-full h-1 bg-[var(--border)] rounded-full mt-1 overflow-hidden">
                        <div
                          className="h-full bg-[var(--system-indigo)] rounded-full"
                          style={{ width: `${50 + i * 15}%` }}
                        />
                      </div>
                    </div>
